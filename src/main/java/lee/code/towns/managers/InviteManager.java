    ...
    Bukkit.getServer().getAsyncScheduler().runDelayed(towns, scheduledTask -> {
      if (hasActiveInvite(playerID, targetID)) {
        ...
        if (oPlayer.isOnline()) {
          ...
          if (player != null)
            player.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.ERROR_INVITE_TIMEOUT_PLAYER.getComponent(new String[]{oTarget.getName()})));
        }
        if (oTarget.isOnline()) {
          final Player target = oTarget.getPlayer();
          if (target != null)
            target.sendMessage(Lang.PREFIX.getComponent(null).append(Lang.ERROR_INVITE_TIMEOUT_TARGET.getComponent(new String[]{oPlayer.getName()})));
        }
